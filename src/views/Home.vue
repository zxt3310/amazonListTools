<template>
  <div class="home">
      ...
      <el-col :span="4" v-for="(item, index) in pc.title_addition" :key="index">
        <el-input v-model="pc.title_addition[index]">
          ...
          <el-button
            ...
            icon="el-icon-delete"
            circle
            @click="title_addition_remove(index)"
          ></el-button>
        </el-input>
      </el-col>
      <el-col :span="1" style="text-align: left;">
        <el-button type="primary" @click="title_addition_add()"
    ...
    <el-row>
      <el-col>
        <el-input
          v-model="descriptions.cpu_description"
          placeholder="10th Generation Intel Core i3-1005G1 processor 1.2GHz with turo boost Technology up to 3.4GHz"
        >
      ...
      <el-col :span="6" v-for="(item, index) in base.disks" :key="index">
        <el-input v-model="base.disks[index].size">
          ...
          <el-button
            ...
            icon="el-icon-delete"
            circle
            @click="base_disk_remove(index)"
          ></el-button>
        </el-input>
      </el-col>
      <el-col :span="1" style="text-align: left;">
        <el-button type="primary" @click="base_disk_add()"
          >增加硬盘<i class="el-icon-circle-plus"></i>
        </el-button>
      </el-col>
    </el-row>
    <el-divider></el-divider>
    ...
    <div style="text-align: left; font-size: 25px; font-weight: 500;">
      Variation:
    </div>
        ...
        <el-col :span="6" v-for="(item_disk, idx) in item.disks" :key="idx">
          <el-input v-model="variations[index].disks[idx].size">
            ...
            <el-button
              ...
              icon="el-icon-delete"
              circle
              @click="variation_disk_remove(index, idx)"
            ></el-button>
          </el-input>
        </el-col>
        <el-col :span="1" style="text-align: left;">
          <el-button type="primary" @click="variation_disk_add(index)"
            >增加硬盘<i class="el-icon-circle-plus"></i>
          </el-button>
        </el-col>
      </el-row>
    ...
    <el-button type="primary" @click="variation_add()"
      >新增variation <i class="el-icon-circle-plus"></i
    ></el-button>
    ...
    <div>
      <el-button
        style="margin-top: 3rem; width: 250px;"
        type="success"
        round
        @click="variation_generate()"
      >
        生成listing
      </el-button>
    </div>
    <div
      style="text-align: left; font-size: 20px; font-weight: 500; margin-bottom: 20px;"
    >
      SKU:
      <el-button
        class="copy_sku"
        type="primary"
        plain
        :data-clipboard-text="sku_copy_str"
        @click="copy_sku()"
        >复制文本</el-button
      >
    </div>
    <div>
      <el-row :gutter="20" v-for="(item, index) in skus" :key="index">
        <el-input v-model="skus[index]" maxlength="40" show-word-limit>
          <template slot="prepend">{{
            index == 0 ? "Base" : "variation" + index
          }}</template>
        </el-input>
      </el-row>
    </div>
    ...
    >
      ...
      <el-button
        ...
        type="primary"
        plain
        :data-clipboard-text="title_copy_str"
        @click="copy_title()"
        >复制文本</el-button
      >
    </div>
    <div>
      <el-row :gutter="20" v-for="(item, index) in titles" :key="index">
        <el-input v-model="titles[index]" maxlength="200" show-word-limit>
          <template slot="prepend">{{
            index == 0 ? "Base" : "variation" + index
          }}</template>
        </el-input>
    ...
    >
      ...
      <el-button
        ...
        type="primary"
        plain
        :data-clipboard-text="bullet_2_copy_str"
        @click="copy_bullet_2()"
        >复制文本</el-button
      >
    </div>
    <el-row :gutter="20" v-for="(item, index) in bullet_2" :key="index">
      <el-input v-model="bullet_2[index]">
    ...
    >
      ...
      <el-button
        ...
        type="primary"
        plain
        :data-clipboard-text="bullet_5_copy_str"
        @click="copy_bullet_5()"
        >复制文本</el-button
      >
    </div>
    <el-row :gutter="20" v-for="(item, index) in bullet_5" :key="index">
      <el-input v-model="bullet_5[index]">
        <template slot="prepend">variation_{{ index + 1 }}</template>
      </el-input>
    </el-row>
    ...
    <el-row
      :gutter="20"
      v-for="(item, index) in html_descriptions"
      :key="index"
    >
      <el-input v-model="html_descriptions[index]">
        <template slot="prepend">variation_{{ index + 1 }}</template>
      </el-input>
    </el-row>
  </div>
    ...
    variation_generate() {
      ...
      switch (pc.for_free) {
        case 1:
          ...
          description_head =
            "<p><strong>Disclaimer: We sell computers with customized configurations. If the computer has modifications different than the base model, the&nbsp;factory box is opened to install the upgrades to achieve the specifications as advertised. The modified product is professionally tested and resealed.</strong></p>";
          description_free =
            "<p><strong>Free&nbsp;32GB USB Drive included</strong></p>";
          break;
        case 2:
          free_bullet_str = "free 32GB USB 3.0 Flash Drive";
          description_head =
            "<p><strong>Rock eDigital&nbsp;sells computers with upgraded configurations. If the computer has modifications (listed above), then the manufacturer box is opened for it to be tested and inspected and to install the upgrades to achieve the specifications as advertised.</strong></p>";
          description_free =
            "<p><strong>Free 32GB USB 3.0 Flash Drive</strong></p>";
          break;
        default:
          free_bullet_str =
            "free Acessories: 1xHDMI cable, 1xUSB Extension cord, 1xMousepad";
          description_head =
            "<p><strong>Rock eDigital&nbsp;sells computers with upgraded configurations. If the computer has modifications (listed above), then the manufacturer box is opened for it to be tested and inspected and to install the upgrades to achieve the specifications as advertised.</strong></p>";
          description_free =
            "<p><strong>Free accessories:</strong><br />1xHDMI cable, 1xUSB Extension cord, 1xMousepad</p>";
          break;
      }
      ...
      let base_title = `${this.get_cur_year()} Newest ${pc.manufacture} ${
        pc.series
      } ${pc.screen} ${pc.pctype} ${pc.cpu} ${pc.graphics} ${
        base.ram_size
      }GB RAM DDR4${base_disk_title_str}${base_title_addition_str} ${system_str}`;
      ...
      for (let vari of this.variations) {
        ...
        //SKU 和 Title
        let var_sku = `${pc.manufacture}-${pc.screen_size}-${pc.model}-${
          vari.ram_size
        }GB${disk_sku_str}${
          vari.system > 1
            ? vari.system == 2
        ...
        let var_title = `${this.get_cur_year()} Newest ${pc.manufacture} ${
          pc.series
        } ${pc.screen} Laptop ${pc.cpu} ${pc.graphics} ${
          vari.ram_size
        }GB RAM DDR4${disk_title_str}${base_title_addition_str} ${system_str} ${
          pc.for_free == 0
            ? "w/Accessories"
            : pc.for_free == 1
            ? "32GB USB Drive"
        ...
        let isUpgrade =
          vari_ram > bass_ram ||
          vari_disk_size > base_disk_size ||
          (vari_disk_size == 1 && vari_disk.type.indexOf("SSD") != -1);

        let bullet_2 = `【Customization】${
          isUpgrade ? "Upgraded to " : ""
        }${vari_ram}GB DDR4 SDRAM, ${disk_title_str}, ${pc.dvd}`;
        this.bullet_2.push(bullet_2);

        let bullet_5_head =
          pc.for_free == 0 || pc.for_free == 2
            ? vari.system == 0 || vari.system != 2
              ? "【Rock eDigital Bundle】"
              : "【Rock eDigital Enhancement】"
            : vari.system == 0 || vari.system != 2
            ? "【For Home】"
            : "【For Business】";
        let bullet_5 = `${bullet_5_head}${
          vari.system <= base.system ? "" : "Upgraded to "
        }${system_str}, ${descriptions.battery_description}, ${
          descriptions.size_description
        }, ${pc.color}, ${free_bullet_str}`;
        this.bullet_5.push(bullet_5);

        //Description

        let html = `${description_head}<p>&nbsp;</p> 
					<p><strong>PC Series:</strong><br />${pc.manufacture} ${pc.series} ${
          pc.pctype
        }</p> 
            ...
            vari_ram > bass_ram ? "<strong>Upgraded to&nbsp;</strong>" : ""
          }${vari_ram}GB DDR4</p>
					<p><strong>Storage:</strong><br />${
            vari_disk_size > base_disk_size ||
            vari.disks.length > base.disks.length ||
            (vari_disk_size == 1 && vari_disk.type.indexOf("SSD") != -1)
            ...
            descriptions.connectivity_description
          }</p>
            ...
            vari.system == 0 ? "" : "<strong>Upgraded to&nbsp;</strong>"
          }${system_str}</p>
					<p><strong>Ports &amp; Slots:</strong><br />${descriptions.solt_description}</p>
					<p><strong>Additional Information:</strong><br />Dimensions:${
            descriptions.size_description
          }</p>
					${description_free}`;
      ...
      this.bullet_1 = `【Powerful Performance with ${
        pc.cpu
      } ${this.getCores_str(pc.cpu_cores)} Core】${
        descriptions.cpu_description
      }`;
