						...
						<div class="notification-container">
							...
							<div class="notification-content">
								<el-table 
									...
									:max-height="Math.min(400, Math.max(200, safeNotifications.length * 80))"
									style="border: none;"
									highlight-current-row
								>
									<el-table-column prop="type" width="100px" show-overflow-tooltip></el-table-column>
									<el-table-column prop="message" width="280px" show-overflow-tooltip></el-table-column>
									<el-table-column width="100px">
										<template slot-scope="scope">
											<div style="display: flex; align-items: center;">
												<el-badge is-dot :hidden="scope.row?.is_read" style="margin-right: 10px;"></el-badge>
								...
								<div v-if="safeNotifications.length === 0" class="empty-notification">
									暂无消息通知
								</div>
							</div>
						</div>
...
<script>
	...
	export default {
		...
		computed: {
			notifications() {
				// 确保返回的是数组，防止null值
				return this.$store && this.$store.state && Array.isArray(this.$store.state.notifications) 
		...
		methods: {
			handleSelect(key, keyPath) {
				// 增强安全检查
				if (!key || typeof key !== 'string' || !this.$router || !this.$router.history || !this.$router.history.current) {
					return;
				}
			...
			markAllAsRead() {
				...
				const unreadIds = this.notifications
					.filter(item => item && typeof item === 'object' && item.is_read === false && item.id)
					.map(item => item.id);
				
				if (unreadIds.length > 0 && this.$store) {
					axios.post("readAllMessages", {
						ids: unreadIds
					}).then((e) => {
						if (e && e.ret == 0) {
							this.$store.dispatch("fetchMessage");
...
<style>
	...
	/* 未读消息样式强调 */
	.el-table .el-table__row:not(.el-table__row--striped):not(.el-table__row--highlight):not([class*="current-row"]) {
